...
{
    case 'listar':
        $datos = mysqli_query($conexion, "SELECT referencia, categoria , marca, modelo, etiqueta, FORMAT(pvp,2,'es_ES') pvp, FORMAT(pvp_clubsave,2,'es_ES') pvp_clubsave FROM repuestos");
    ...
    case 'agregar':
        $respuesta = mysqli_query( $conexion,"INSERT INTO repuestos (referencia, marca, categoria, modelo, etiqueta, pvp, pvp_clubsave) VALUES ('$_POST[referencia]', '$_POST[marca]', '$_POST[categoria]', '$_POST[modelo]' ,'$_POST[etiqueta]', '$_POST[pvp]', '$_POST[pvp_clubsave]')");
    ...
    case 'consultar':
        $datos = mysqli_query($conexion, "SELECT referencia, categoria , marca, modelo, etiqueta, pvp, pvp_clubsave  FROM repuestos WHERE referencia='$_GET[referencia]'");
        $resultado= mysqli_fetch_all($datos, MYSQLI_ASSOC);
        echo json_encode($resultado);
        mysqli_close($conexion);
        break;
    ...
    case 'modificar':
        $respuesta = mysqli_query($conexion,"UPDATE repuestos SET categoria = '$_POST[categoria]',
                                                                    ...
                                                                    pvp = '$_POST[pvp]',
                                                                    pvp_clubsave = '$_POST[pvp_clubsave]'
